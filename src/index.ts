...
import {RefactorSessionChainable, isLiteral, isStatement, copy} from 'shift-refactor';
...
export function unsafeMethods() {
  return {
    findPureFunctionCandidates(
      ...
      options?: PureFunctionAssessmentOptions,
    ): Map<FunctionDeclaration, PureFunctionAssessment> {
      return new Map(
        ...
        (decl: VariableDeclarator | FunctionDeclaration | ClassDeclaration) => {
          ...
          if (reads.length === 0) {
            lookup.references.forEach((ref: Reference) => {
              ...
              if (
                ['VariableDeclarator', 'FunctionDeclaration', 'ClassDeclaration'].indexOf(immediateParent.type) > -1
              ) {
